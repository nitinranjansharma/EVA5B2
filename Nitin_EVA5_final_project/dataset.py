...
class create_data(Dataset):
    ...
    def __init__(self,yolo_params,planercnn_params,midas_params):
        ...
        #print(yolo_params)
        #print(planercnn_params)
        #print(midas_params)
        print(self.options.customDataFolder)
        ...
        for im in self.img_files:
            ...
            np_file[4] = np_file[4].replace(os.path.splitext(np_file[4])[-1], '_plane_parameters_0.npy')
            np_file2[4] = np_file2[4].replace(os.path.splitext(np_file2[4])[-1], '_plane_masks_0.npy')
            im = os.sep.join(im)
            np_file = os.sep.join(np_file)
            np_file2 = os.sep.join(np_file2)
            np_list = [np_file,np_file2]
            self.plane_names.append(im)
            self.plane_nps.append(np_list)
    ...
    def __getitem__(self,index):
        ...
        #print(plane_np_name)
        try:
            plane_np = {'plane_parameters':np.load(plane_np_name[0]),'plane_masks':np.load(plane_np_name[1])}
